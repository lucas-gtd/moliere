...
const validateExecutable = (
  ...
  if (!command.trim()) throw new ToolInputError('"command" ne peut pas etre vide.');
  if (command.includes("\0")) {
    throw new ToolInputError('"command" contient un caractere nul interdit.');
  }

  const executableName = path.basename(command);
  if (BLOCKED_COMMANDS.has(executableName)) {
    throw new ToolInputError(`Commande bloquee pour securite: ${executableName}.`);
  }
  ...
  if (!/^[a-zA-Z0-9._+-]+$/.test(command)) {
    throw new ToolInputError(
      '"command" doit etre un executable simple, sans shell ni metacaracteres.',
    );
  }
...
const validateCommandArgs = (command: string, args: string[]) => {
  ...
  if (
    ...
    throw new ToolInputError(
      "Execution inline (-e/--eval/-c) bloquee. Cree un fichier de projet puis execute-le.",
    );
  }

  if (executableName === "git" && args[0] && BLOCKED_GIT_SUBCOMMANDS.has(args[0])) {
    throw new ToolInputError(
      `Sous-commande git bloquee pour securite: ${args[0]}. Utilise les outils git dedies.`,
    );
  }

  for (const arg of args) {
    if (arg.length > 4_000) {
      throw new ToolInputError("Un argument de commande depasse 4000 caracteres.");
    }
  }
};
...
const runCommandTool: ToolDefinition = {
  name: "runCommand",
  description:
    "Execute une commande locale sans shell, limitee au projet, avec timeout et sortie plafonnee. Utile pour tests/builds.",
  access: "command",
  parameters: {
    type: "object",
    ...
    properties: {
      command: {
        type: "string",
        description: "Executable a lancer, ex: bun, npm, node, git",
      },
      args: {
        type: "array",
        items: { type: "string" },
        description: "Arguments separes. Aucun shell n'est utilise.",
      },
      cwd: {
        type: "string",
        description: "Dossier de travail dans le projet. Defaut: racine",
      },
      timeoutMs: {
        type: "integer",
      ...
      maxOutputChars: {
        type: "integer",
        description: "Nombre maximum de caracteres stdout/stderr retournes",
      },
    },
    required: ["command"],
  },
